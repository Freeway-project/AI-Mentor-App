            ...
            <div className="container mx-auto px-4 md:px-6">
                ...
                <div className="grid gap-8 md:grid-cols-3">
                    {testimonials.map((testimonial, index) => (
                        <Card key={index} className="bg-slate-900 border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                            <CardContent className="p-8 space-y-6 flex flex-col h-full">
                                <Quote className="h-8 w-8 text-amber-500/40" />
                                <p className="text-slate-300 italic flex-1">
                                    &ldquo;{testimonial.quote}&rdquo;
                                </p>
                                <div className="flex items-center gap-4 pt-4 border-t border-slate-800">
                                    <div className="h-10 w-10 rounded-full overflow-hidden bg-slate-800 shrink-0">
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={testimonial.image} alt={testimonial.author} className="h-full w-full object-cover opacity-90" />
                                    </div>
