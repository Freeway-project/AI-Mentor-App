...
class ApiClient {
  ...
  async getSessionToken(meetingId: string): Promise<{ token: string; serverUrl: string; roomName: string }> {
    return this.request<{ token: string; serverUrl: string; roomName: string }>(`/sessions/${meetingId}/token`);
  }
