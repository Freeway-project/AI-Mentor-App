...
export default function MentorMessagesPage() {
  ...
  const loadMessages = useCallback(async (id: string, silent = false) => {
    ...
    try {
      ...
      setConversations(prev =>
        prev.map(c => c.id === id ? { ...c, unreadCount: { ...c.unreadCount, mentor: 0 } } : c)
      );
    } catch {
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-5xl px-4 py-6 sm:px-6 md:py-8 md:px-8 space-y-6">
      <AppPageHeader
        title="Messages"
        description="Inquiry threads from your mentees."
      />
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
            {conversations.length === 0 ? (
              ...
              conversations.map(conv => {
                const unread = conv.unreadCount?.mentor ?? 0;
                return (
                  <button
                    key={conv.id}
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-medium text-slate-800 truncate">{conv.menteeName ?? 'Mentee'}</p>
                      {unread > 0 && (
                        <span className="shrink-0 rounded-full bg-amber-500 px-1.5 py-0.5 text-[10px] font-bold text-white">{unread}</span>
                      )}
                    </div>
                    {conv.lastMessageAt && (
          ...
          {!activeId ? (
            ...
            <>
              <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-slate-100">
                <p className="font-semibold text-slate-800 text-sm">{activeConv?.menteeName ?? 'Mentee'}</p>
                <button
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                ...
                {messages.map(msg => {
                  const isMine = msg.senderRole === 'mentor';
                  return (
              ...
              <div className="border-t border-slate-100 px-4 py-3 flex gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl bg-amber-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-amber-500 disabled:opacity-50"
                >
